...
```{r}
surxondarya_input_01 <- readxl::read_xlsx(here("data", "surxondarya.xlsx")) 
```
...
```{r}
surxondarya_input_02 <- surxondarya_input_01 %>% 
  select(-1, -4) %>% 
  set_names(new_names) %>% 
...
```{r}
skim(surxondarya_input_02)
```

```{r}
q1_reg <- reg_table(surxondarya_input_02, q_1) %>% 
...
q2_reg <- reg_table(surxondarya_input_02, q_2) %>% 
...
q3_reg <- reg_table(surxondarya_input_02, q_3) %>% 
...
q4_reg <- reg_table(surxondarya_input_02, q_4) %>% 
...
q5_reg <- reg_table(surxondarya_input_02,q_5) %>% 
...
q6_reg <- reg_table(surxondarya_input_02, q_6) %>% 
...
q7 <- reg_table(surxondarya_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "surxondarya_indeks_2023_01_01.xlsx", overwrite = TRUE) 
```
